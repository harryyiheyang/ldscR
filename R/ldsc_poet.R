#' Estimate Heritability and Genetic Correlation Matrix Using LDSC estimated from POET LD matrix estimate
#'
#' The \code{ldsc_poet} function estimates heritability and genetic correlation matrices using Linkage Disequilibrium Score Regression (LDSC). The sample LD matrix is improved by the Principal Orthogonal complEment Thresholding (POET) method.
#'
...
#' ref_panel <- hapmap3
#' LDSC <- EURLDSC
#' results <- ldsc_poet(GWAS_List, LDSC)
#'
...
ldsc_poet=function(GWAS_List,LDSC){
  ...
  l = LDSC1$LDSC.P * sqrt(NMatrix1[[col_names[i+1]]]/M) * sqrt(NMatrix1[[col_names[j+1]]]/M)
  fit0=lm(z~l)
  ...
  l = LDSC1$LDSC.P * sqrt(NMatrix1[[col_names[i+1]]]/M) * sqrt(NMatrix1[[col_names[i+1]]]/M)
  w=(1+l*GCovEst[i,i])^2
  ...
  z = ZMatrix1[[col_names[i+1]]] * ZMatrix1[[col_names[j+1]]]
  l = LDSC1$LDSC.P * sqrt(NMatrix1[[col_names[i+1]]]/M) * sqrt(NMatrix1[[col_names[j+1]]]/M)
  li = LDSC1$LDSC.P * NMatrix1[[col_names[i+1]]]/M
  lj = LDSC1$LDSC.P * NMatrix1[[col_names[j+1]]]/M
